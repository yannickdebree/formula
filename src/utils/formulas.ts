...
export function getPointsToDrawFromFormula(formula: Formula, canvasState: CanvasState) {
    return new Promise<Array<PointToDraw>>(resolve => {
        const pointsToDraw = new Array<PointToDraw>();

        const fn = new Function(`return function(x) { return ${formula.content.toLowerCase().replace(/\s/g, '').trim()}; }`)() as (x: number) => number;
...
export function getPointsToDrawFromFormulas(formulas: Array<Formula>, canvasState: CanvasState) {
    return Promise.all(
